...
EXPERIMENT_FINISHED_PATTERN = rf"({TIMESTAMP_REGEX})  INFO .*? User -> (.*?) , Endpoint.*?Experiment finished: .*?uuid=(.*?), name.*?, status=(.*?), result.*?, finished=(.*?), algorithm=(.*?), algorithmId.*? created=(.*?), updated.*?"
...
def print_audit_entry(log_line):
    if pattern_groups := re.search(EXPERIMENT_FINISHED_PATTERN, log_line):
        ...
        print(
            f"{log_timestamp} - {user} - EXPERIMENT_FINISHED - {uuid} - {algorithm_properties['name']} - {parameters['pathology']} - {parameters['dataset']} - {status} - {exp_time_to_finish} - {parameters} - {algorithm_properties['preprocessing']}"
        )
