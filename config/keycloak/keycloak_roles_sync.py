...
from keycloak import raise_error_from_response, KeycloakGetError
from keycloak import URL_ADMIN_CLIENT_ROLE
...
class MIPKeycloakAdmin(KeycloakAdmin):
    ...
    def get_composite_client_roles_of_role(self, client_id, role_name):
        return self._get_composite_client_roles_of_role(
            URL_ADMIN_CLIENT_ROLE + "/composites", client_id, role_name
        )

    def _get_composite_client_roles_of_role(
        self, client_roles_composite_url, client_id, role_name
    ):
        params_path = {
            "id": client_id,
            ...
            if role is not None:
                if role["composite"] is True:
                    if client_id is None:
                        ...
                        roles = self.get_composite_client_roles_of_role(
                            client_id, role_name
                        )
        else:
            if client_id is None:
    ...
    def roles_list_to_dict(self, roles_list, depth=0):
        ...
        for role in roles_list:
            ...
            if "composites" in role and role["name"] not in sorted_roles_dict:
                sorted_roles_dict[role["name"]], children = self.roles_list_to_dict(
                    role["composites"], depth + 1
                )
                if len(children) > 0:
    ...
    def synchronize_client(self, client_id, pathologies):
        ...
        if client is not None:
            ...
            sys.exit(
                "Client <%s> does not exist and cannot be created either!" % (client_id)
            )
        ...
        client_roles = [
            {
                "name": "DC_ADMIN",
                "composite": False,
                "description": "Data catalog administrator",
            },
            {
                "name": "RESEARCH_DATASET_ALL",
                "composite": False,
                "description": "All datasets, without distinction",
            },
            {"name": "RESEARCH_EXPERIMENT_ALL", "composite": False},
            {
                "name": "RESEARCH_PATHOLOGY_ALL",
                "composite": False,
                "description": "All pathologies, without distinction",
            },
            {
                "name": "WORKFLOW_ADMIN",
                "composite": False,
                "description": "Galaxy administrator",
            },
        ]
        ...
        for pathology in pathologies:
            dc_control_role = {
                "name": "DC_CONTROL_%s" % (pathology["name"]),
                "composite": False,
                "description": "Data catalog - %s CDE" % (pathology["name"]),
            }
            ...
            pathology_composite_role = {
                "name": "RESEARCH_PATHOLOGY_%s" % (pathology["name"]),
                "composite": True,
                "description": "%s pathology composite role" % (pathology["name"]),
            }

            for node in pathology["nodes"]:
                dc_hospital_role = {
                    "name": "DC_HOSPITAL_%s" % (node["name"]),
                    "composite": False,
                    "description": "Data catalog - Worker %s hospital" % (node["name"]),
                }
                ...
                for dataset in node["datasets"]:
                    dataset_role = {
                        "name": "RESEARCH_DATASET_%s" % (dataset["name"]),
                        "composite": False,
                        "description": "%s - Worker %s"
                        % (pathology["name"], node["name"]),
                    }
                    if dataset_role["name"] not in [
                        pathology_dataset_role["name"]
                        for pathology_dataset_role in full_pathology_role_list
                    ]:
        ...
        if client_id_id is not None:
            ...
            if len(old_client_roles) > 0:
                if self._verbose_level >= 2:
                    print(
                        "        Deleting existing <%s> client roles...\n" % (client_id)
                    )
                for old_role in old_client_roles:
                    result = self.delete_client_role(
                        client_role_id=client_id_id, role_name=old_role["name"]
                    )
            ...
            for new_role in client_roles:
                ...
                if self._verbose_level >= 2:
                    print(
                        "        Creating <%s> client role: %s (composite: %s)"
                        % (client_id, new_role["name"], new_role["composite"])
                    )
                result = self.create_client_role(
                    client_role_id=client_id_id, payload=new_role, skip_exists=False
                )
                if new_role["composite"] and new_role["name"] in pathologies_datasets:
                    ...
                    for composite in pathologies_datasets[new_role["name"]]:
                        if self._verbose_level >= 3:
                            print(
                                "            Assigning <%s> client role <%s> as a composite of <%s> role"
                                % (client_id, composite["name"], new_role["name"])
                            )
                        composite_role = self.get_client_role(
                            client_id_id, composite["name"]
                        )
                        composites.append(composite_role)
                    if len(composites) > 0:
                        result = self.add_composite_client_roles_to_role(
                            client_role_id=client_id_id,
    ...
    def copy_group_members(self, client_id, copy_from_client_id, group_name):
        ...
        if src_members is not None:
            ...
            if dst_group is not None:
                ...
                if self._verbose_level >= 3:
                    print(
                        "            Copying missing users from source group <%s> to destination group <%s>"
                        % (src_group_name, dst_group_name)
                    )
                for src_member in src_members:
                    if dst_members is None or src_member["id"] not in [
                        dst_member["id"] for dst_member in dst_members
                    ]:
    ...
    def _assign_client_roles_to_group(self, client_id, group_name):
        ...
        if client_id_id is not None and dst_group is not None:
            ...
            if group_name == "DC_ADMIN":
                ...
                role_names_list = [
                    "RESEARCH_DATASET_ALL",
                    "RESEARCH_EXPERIMENT_ALL",
                    "RESEARCH_PATHOLOGY_ALL",
                ]
            elif group_name.startswith("CONTROLLER_"):
                pathology = group_name[11:]
                role_names_list = [
                    "DC_CONTROL_%s" % (pathology),
                    "RESEARCH_PATHOLOGY_%s" % (pathology),
                ]
            ...
            if len(roles_list) > 0:
                for role in roles_list:
                    if self._verbose_level >= 3:
                        print(
                            "            Assigning <%s> client role <%s> to realm group: <%s>"
                            % (client_id, role["name"], dst_group_name)
                        )
                self.assign_group_client_roles(
                    dst_group["id"], client_id_id, roles_list
                )
    ...
    def _group_members_backup(self, group_id):
        ...
        if os.path.isfile(
            self._group_members_backup_filename
        ):  # If we already find the file at the backup time, we should suspect a previous crash and may find a group members backup inside
            try:
        ...
        if (
            ...
            and len(group_members) > 0
            and group["name"] not in members_backup
        ):
        ...
        if members_backup is not None:
            try:
                with open(self._group_members_backup_filename, "w") as f:
                    if self._verbose_level >= 1:
                        print(
                            "        Saving users from destination group <%s> into backup file: <%s>"
                            % (group["name"], self._group_members_backup_filename)
                        )
    ...
    def _group_members_restore(self, group_id):
        ...
        if members_backup is not None and group["name"] in members_backup:
            try:
                ...
                if self._verbose_level >= 1:
                    print(
                        "            Restoring missing users from backup file <%s> to destination group: <%s>"
                        % (self._group_members_backup_filename, group["name"])
                    )
                ...
                if len(members_backup) == 0 and os.path.isfile(
                    self._group_members_backup_filename
                ):
                    os.remove(self._group_members_backup_filename)
                result = True
    ...
    def consolidate_groups(self, client_id, pathologies, copy_from_client_id=None):
        ...
        for group in old_client_matching_groups:
            ...
            for pattern in group_patterns:
                ...
                if group_name.startswith(pattern):
                    match = True
                    break
            if not match and group["name"] not in [
                group["name"] for group in self._orphan_groups
            ]:
        ...
        for base_group_name in group_patterns:
            if base_group_name == "DC_HOSPITAL":
                    ...
                    for node in pathology["nodes"]:
                        if node["name"] not in nodes:
                            ...
                            self._consolidate_group(
                                client_id, group_name, copy_from_client_id
                            )
                            sync_client_group_names.append(
                                "%s_%s" % (client_id.upper(), group_name)
                            )
                ...
                sync_client_group_names.append(
                    "%s_%s" % (client_id.upper(), base_group_name)
                )

        for group in old_client_matching_groups:
            if group["name"] not in sync_client_group_names and group["name"] not in [
                group["name"] for group in self._orphan_groups
            ]:
                self._orphan_groups.append(group)
        ...
        if default_groups is not None:
            ...
            if restore_default_groups:
                ...
                for group in default_groups:
                    if not group.startswith("/"):
                        group = "/" + group
                    if group not in self._realm["defaultGroups"]:
                ...
                if len(missing_default_groups) > 0:
                    ...
                    for group in missing_default_groups:
                        if self._verbose_level >= 0:
                            print(
                                "  THE FOLLOWING DEFAULT GROUP IS NOW MISSING!! YOU REALLY SHOULD RESTORE IT MANUALLY RIGHT NOW: %s"
                                % (group)
                            )
...
def main():
    ...
    argsparser.add_argument(
        ...
        "--server-url",
        dest="server_url",
        default="https://iam.ebrains.eu",
        help="Keycloak server base URL",
        type=str,
    )
    argsparser.add_argument(
        "-r",
    ...
    argsparser.add_argument(
        ...
        help="Keycloak Realm Administrator client-id",
        type=str,
    )
    argsparser.add_argument(
        "-k",
        "--admin-client-secret",
        dest="admin_client_secret",
        help="Keycloak Realm Administrator client-secret",
        type=str,
    )
    argsparser.add_argument(
        "-u",
        ...
        dest="admin_username",
        default="realmadmin",
        help="Keycloak Realm Administrator username",
        type=str,
    )
    argsparser.add_argument(
        "-p",
        "--admin-password",
        dest="admin_password",
        help="Keycloak Realm Administrator password",
        type=str,
    )
    argsparser.add_argument(
        "-s",
        "--sync-client-id",
        dest="sync_client_id",
        help="Keycloak Realm client-id to create/synchronize. Commonly MIP federation name. Will be used as group prefix as well.",
        type=str,
    )
    argsparser.add_argument(
        "-c",
        "--copy-users-from-client-id",
        dest="copy_from_client_id",
        help="If you want to copy the user/group relationships from another client/group_prefix",
        type=str,
    )
    argsparser.add_argument(
        "-i",
        "--data-structure-json-file",
        dest="data_structure_json_file",
        default=sys.stdin,
        nargs="?",
        help="JSON data structure file to use as a pattern for roles/groups creation/synchronization",
        type=argparse.FileType("r"),
    )
    argsparser.add_argument(
        "-v", dest="verbose_level", action="count", default=0, help="Verbose level"
    )
    ...
    if pathologies is not None:
        try:
            keycloak_adm = MIPKeycloakAdmin(
                server_url=args["server_url"] + "/auth/",
    ...
    if keycloak_adm is not None:
        if args["verbose_level"] >= 0:
            print(
                "Synchronizing <%s> client in Keycloak <%s> realm (on server <%s>), connecting with realm admin user <%s> (realm admin client <%s>)..."
                % (
                    args["sync_client_id"],
                    args["realm_name"],
                    args["server_url"],
        ...
        keycloak_adm.consolidate_groups(
            args["sync_client_id"],
            pathologies,
            copy_from_client_id=args["copy_from_client_id"],
        )
        ...
        if len(orphan_groups) > 0:
            ...
            for orphan_group in orphan_groups:
                if args["verbose_level"] >= 0:
                    print(
                        "WATCH OUT! Orphan group (does not match the client group naming convention). You may be wanting to delete it: %s"
                        % (orphan_group["name"])
                    )
