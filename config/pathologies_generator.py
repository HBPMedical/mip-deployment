...
class gen_pathologies:
    ...
    def __metadata_labels_processor(self, content, allowed_change=False):
        ...
        for item in content:
            ...
            if isinstance(item_content, dict) or isinstance(item_content, list):    # Recursive calls
                if item_name is not None and item_name == 'groups':
                    # Recursive call in <groups>
                    item_content, tmp_content_changes = self.__metadata_labels_processor(item_content, True)
                    if tmp_content_changes:
                        content_changes = True
                else:
                    if isinstance(content, list):
                        if allowed_change:      # We're parsing a list of groups
                            # Recursive call in the item of the groups list
                            item_content, tmp_content_changes = self.__metadata_labels_processor(item_content, True)
                            if tmp_content_changes:
                                content_changes = True
                        else:
                            # Recursive call in the list's item
                            item_content, tmp_content_changes = self.__metadata_labels_processor(item_content)
                            if tmp_content_changes:
                                content_changes = True
                    else:
                        # Recursive call in the item
                        item_content, tmp_content_changes = self.__metadata_labels_processor(item_content)
                        if tmp_content_changes:
                            content_changes = True
            ...
            if isinstance(content, dict):
                if item_name == 'code':
                    current_code = item_content
                elif item_name == 'label':
                    current_label = item_content

        if allowed_change and current_code is not None:
            modified_label = current_label

            if current_label is None:       # code without label
                modified_label = current_code

            modified_label = modified_label.replace('_', ' ')
            modified_label = ' '.join(modified_label.split())
            modified_label = modified_label.title()
            if modified_label != current_label: # We actually modified the label
                content_changes = True
                content['label'] = modified_label

        return content, content_changes
    ...
    def __metadata_processor(self, pathology):
        ...
        if not self.__config['data']['metadata_keep_group_labels']:
            # Potentially modifying the source CDEs
            source_metadata_changes = False
            self.__metadatas[pathology]['source'], source_metadata_changes = self.__metadata_labels_processor(self.__metadatas[pathology]['source'])
            if source_metadata_changes:
                metadata_dataset_file_change = True

            # Potentially modifying the final CDEs (it will only impact the pathologies.json, not the original CDEs)
            final_metadata_changes = False
            self.__metadatas[pathology]['final'], final_metadata_changes = self.__metadata_labels_processor(self.__metadatas[pathology]['final'])
...
def main():
    ...
    argsparser.add_argument('--data-path', dest='data_path', help='Data base directory path', type=str)
    argsparser.add_argument('--max-recursion-depth', dest='data_max_recursion_depth', default=1, help='Maximum level of recursion for data directory analysis', type=int)
    argsparser.add_argument('--force-relabel', dest='data_force_relabel', default=False, action='store_true', help='Force relabeling pathologies and datasets in CDE metadata files and pathologies file')
    argsparser.add_argument('--metadata-keep-group-labels', dest='data_metadata_keep_group_labels', default=False, action='store_true', help='Do not fix the groups labeling in CDE metadata files')
    argsparser.add_argument('--metadata-online-sync', dest='data_metadata_online_sync', default=False, action='store_true', help='Download CDE metadata content from online Data Catalogue')
    argsparser.add_argument('--metadata-datacatalogue-api-url', dest='data_metadata_datacatalogue_api_url', default='http://datacatalogue.mip.ebrains.eu:8086', help='Online CDE metadata datacatalogue API URL', type=str)
    argsparser.add_argument('--metadata-format', dest='data_metadata_format', default='json', help='CDE metadata files format', type=str)
    argsparser.add_argument('--metadata-filename', dest='data_metadata_filename', default='CDEsMetadata.json', help='CDE metadata file name', type=str)
    argsparser.add_argument('--metadata-encoding', dest='data_metadata_encoding', default='utf-8', help='CDE metadata files encoding', type=str)
    argsparser.add_argument('--metadata-indent-char', dest='data_metadata_indent_char', default=' ', help='CDE metadata files indentation character', type=str)
    argsparser.add_argument('--metadata-indent-count', dest='data_metadata_indent_count', default=2, help='CDE metadata files indentation character count', type=int)
    argsparser.add_argument('--metadata-dataset-unsync', dest='data_metadata_dataset_unsync', default=False, action='store_true', help='Do NOT synchronize CDE metadata files datasets enumeration with dataset files content')
    argsparser.add_argument('--dataset-format', dest='data_dataset_format', default='csv', help='Dataset files format', type=str)
    argsparser.add_argument('--dataset-encoding', dest='data_dataset_encoding', default='ascii', help='Dataset files encoding', type=str)
    argsparser.add_argument('--dataset-delimiter', dest='data_dataset_delimiter', default=',', help='Dataset files fields delimiter', type=str)
    argsparser.add_argument('--dataset-quotechar', dest='data_dataset_quotechar', default='"', help='Dataset files fields quote character', type=str)
    argsparser.add_argument('--pathologies-path', dest='pathologies_path', help='Directory path where to save pathologies file', type=str)
    argsparser.add_argument('--pathologies-format', dest='pathologies_format', default='json', help='Pathologies file format', type=str)
    argsparser.add_argument('--pathologies-filename', dest='pathologies_filename', default='pathologies.json', help='Pathologies file name', type=str)
    argsparser.add_argument('--pathologies-encoding', dest='pathologies_file_encoding', default='utf-8', help='Pathologies file encoding', type=str)
    argsparser.add_argument('--pathologies-indent-char', dest='pathologies_file_indent_char', default=' ', help='Pathologies file indentation character', type=str)
    argsparser.add_argument('--pathologies-indent-count', dest='pathologies_file_indent_count', default=2, help='Pathologies file indentation character count', type=int)
    argsparser.add_argument('--pathologies-preserve-dataset-var', dest='pathologies_preserve_dataset_var', default=False, action='store_true', help='Preserve dataset variable in pathologies file')
    argsparser.add_argument('--pathologies-mip5-ok', dest='pathologies_mip5_compatibility', default=False, action='store_true', help='Generate MIP5 compatible pathologies file')
