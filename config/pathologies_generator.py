...
class gen_pathologies:
    ...
    def __init__(self, args):
        ...
        config["data"]["metadata_datacatalogue_headers"] = {
                ...
                for replacement in replacements:
                    if new_content is None:
                        new_content = content.replace(
                            replacement["search_value"], replacement["replace_value"]
                        )
                    else:
    ...
    def __json_file_writer(self, filepath, content, file_encoding, file_indent):
        ...
        file_indent = (
            file_indent.replace("\\t", "\t").replace("\\n", "\n").replace("\\r", "\r")
        )
        with open(filepath, "wb") as f:
            f.write(
                json.dumps(
                    content, indent=file_indent, ensure_ascii=file_ensure_ascii
    ...
    def __file_writer(
        ...
        if file_format == "json":
            ...
            print(
                'WARNING: No pathologies processor for format "%s"! I won\'t be able to write %s'
                % (file_format, filepath)
            )

    def __metadata_file_writer(self, pathology):
        filepath = os.path.join(
            self.__config["data"]["path"],
            pathology,
            self.__config["data"]["metadata_filename"],
        )
    ...
    def __metadata_processor(self, pathology):
        ...
        for item in ["code", "label"]:
            if item == "code" and content[item] == "root":
                ...
                label = self.__relabel(
                    label, "Relabel pathology '%s'? [%s] " % (pathology, label)
                )
        ...
        for dataset in self.__datasets_codes[pathology]:
            if dataset in self.__get_filtered_dict_list(metadata_datasets, "code"):
                ...
                newlabel = self.__relabel(
                    label,
                    "Pathology '%s': Relabel dataset '%s'? [%s] "
                    % (pathology, dataset, label),
                )
                if newlabel != label:
        ...
        for missing_dataset in missing_datasets:
            missing_dataset_id = self.__get_dict_id(
                self.__datasets[pathology], "code", missing_dataset
            )
            metadata_datasets.append(self.__datasets[pathology][missing_dataset_id])
            if self.__config["data"]["metadata_dataset_sync"]:
                metadata_dataset_variable_id = self.__get_dict_id(
                    self.__metadatas[pathology]["source"]["variables"],
                    "code",
                    "dataset",
                )
                self.__metadatas[pathology]["source"]["variables"][
                    metadata_dataset_variable_id
                ]["enumerations"].append(self.__datasets[pathology][missing_dataset_id])
                metadata_dataset_file_change = True
        for changed_dataset in changed_datasets:
            changed_dataset_id = self.__get_dict_id(
                self.__datasets[pathology], "code", changed_dataset
            )
            metadata_dataset_variable_id = self.__get_dict_id(
                self.__metadatas[pathology]["source"]["variables"], "code", "dataset"
            )
            for i, dataset in enumerate(
                ...
                if dataset["code"] == changed_dataset:
                    self.__metadatas[pathology]["source"]["variables"][
                        metadata_dataset_variable_id
                    ]["enumerations"][i] = self.__datasets[pathology][
                        changed_dataset_id
                    ]
                    self.__metadatas[pathology]["final"]["variables"][
                        metadata_dataset_variable_id
                    ]["enumerations"][i] = self.__datasets[pathology][
                        changed_dataset_id
                    ]
    ...
    def __dataset_processor(self, pathology, file_path):
        ...
        with open(file_path, newline="", encoding=file_encoding) as dataset_file:
            if self.__config["data"]["dataset_format"] == "csv":
                datasetreader = csv.reader(
                    dataset_file,
                    delimiter=self.__config["data"]["dataset_delimiter"],
                    quotechar=self.__config["data"]["dataset_quotechar"],
                )
                for row_id, row in enumerate(datasetreader):
                    if row_id == 0:
                        ...
                        if dataset_field_id is None:
                            print(
                                'WARNING!! Dataset parsing error: Unable to locate "dataset" field in file "%s"!'
                                % file_path
                            )
                            break
                ...
                print(
                    'No dataset processor for format "%s"!'
                    % self.__config["data"]["dataset_format"]
                )
                sys.exit(1)
        if datasets:
                ...
                try:
                    with open(path, encoding=file_encoding) as f:
                        ...
                        if self.__config["data"]["metadata_format"] == "json":
                            ...
                            metadata_content = self.__replace_values(
                                metadata_content, None, file_encoding
                            )
                        else:
                            print(
                                'No metadata processor for format "%s"!'
                                % self.__config["data"]["metadata_format"]
                            )
                            sys.exit(1)
                        if metadata_content is not None:
                            self.__metadatas[pathology]["source"] = copy.deepcopy(
                                metadata_content
                            )
    ...
    def pathologies_generator(self):
        for pathology in self.__pathologies_ids:
            ...
            if self.__metadatas[pathology]["final"] is not None:
                ...
                self.__pathologies[pathology_id][hierarchyName] = self.__metadatas[
                    pathology
                ]["final"]
    ...
    def pathologies_file_writer(self):
        filepath = os.path.join(
            self.__config["pathologies"]["path"],
            self.__config["pathologies"]["filename"],
        )
        ...
        if path is not None and recursion_depth is not None:
            ...
            if os.path.isdir(path):
                if recursion_depth == 1:
                    ...
                    self.__pathologies_ids[current_path_element] = (
                        len(self.__pathologies) - 1
                    )
                    self.__metadatas[current_path_element] = {
                        "source": None,
                        "final": None,
                    }
                    if self.__config["data"]["metadata_online_sync"]:
                        ...
                        if (
                            self.__config["data"]["metadata_datacatalogue_version"]
                            is not None
                        ):
                            metadata_version = self.__config["data"][
                                "metadata_datacatalogue_version"
                            ]
                        datacatalogue_uri = "/pathology/allPathologies/%s/%s" % (
                            current_path_element,
                            metadata_version,
                        )
                        # Old datacatalogue API URL was: http://195.251.252.222:2448
                        response = requests.get(
                            self.__config["data"]["metadata_datacatalogue_api_url"]
                            + datacatalogue_uri,
                            headers=self.__config["data"][
                                "metadata_datacatalogue_headers"
                            ],
                        )
                        if response.status_code == 200:
                            ...
                            if content is not None:
                                if os.path.isfile(
                                    os.path.join(
                                        path, self.__config["data"]["metadata_filename"]
                                    )
                                ):
                                    os.rename(
                                        os.path.join(
                                ...
                                self.__file_writer(
                                    ...
                                    "json",
                                    file_encoding,
                                    self.__config["data"]["metadata_indent"],
                                )
...
def main():
    ...
    argsparser.add_argument(
        "-d", "--data-path", dest="data_path", help="Data base directory path", type=str
    )
    argsparser.add_argument(
        "-r",
        "--max-recursion-depth",
        dest="data_max_recursion_depth",
        default=1,
        help="Maximum level of recursion for data directory analysis",
        type=int,
    )
    argsparser.add_argument(
        "-w",
        "--force-relabel",
        dest="data_force_relabel",
        default=False,
        action="store_true",
        help="Force relabeling pathologies and datasets in CDE metadata files and pathologies file",
    )
    argsparser.add_argument(
        "-s",
        "--metadata-online-sync",
        dest="data_metadata_online_sync",
        default=False,
        action="store_true",
        help="Download CDE metadata content from online Data Catalogue",
    )
    argsparser.add_argument(
        "-z",
        "--metadata-datacatalogue-api-url",
        dest="data_metadata_datacatalogue_api_url",
        default="http://datacatalogue.mip.ebrains.eu:8086",
        help="Online CDE metadata datacatalogue API URL",
        type=str,
    )
    argsparser.add_argument(
        "-f",
        "--metadata-format",
        dest="data_metadata_format",
        default="json",
        help="CDE metadata files format",
        type=str,
    )
    argsparser.add_argument(
        "-m",
        "--metadata-filename",
        dest="data_metadata_filename",
        default="CDEsMetadata.json",
        help="CDE metadata file name",
        type=str,
    )
    argsparser.add_argument(
        "-e",
        "--metadata-encoding",
        dest="data_metadata_encoding",
        default="utf-8",
        help="CDE metadata files encoding",
        type=str,
    )
    argsparser.add_argument(
        "-i",
        "--metadata-indent-char",
        dest="data_metadata_indent_char",
        default=" ",
        help="CDE metadata files indentation character",
        type=str,
    )
    argsparser.add_argument(
        "-c",
        "--metadata-indent-count",
        dest="data_metadata_indent_count",
        default=2,
        help="CDE metadata files indentation character count",
        type=int,
    )
    argsparser.add_argument(
        "-u",
        "--metadata-dataset-unsync",
        dest="data_metadata_dataset_unsync",
        default=False,
        action="store_true",
        help="Do NOT synchronize CDE metadata files datasets enumeration with dataset files content",
    )
    argsparser.add_argument(
        "-t",
        "--dataset-format",
        dest="data_dataset_format",
        default="csv",
    ...
    argsparser.add_argument(
        "-o",
        "--dataset-encoding",
        dest="data_dataset_encoding",
        default="ascii",
        help="Dataset files encoding",
        type=str,
    )
    argsparser.add_argument(
        "-l",
        "--dataset-delimiter",
        dest="data_dataset_delimiter",
        default=",",
        help="Dataset files fields delimiter",
        type=str,
    )
    argsparser.add_argument(
        "-q",
        "--dataset-quotechar",
        dest="data_dataset_quotechar",
        default='"',
        help="Dataset files fields quote character",
        type=str,
    )
    argsparser.add_argument(
        "-p",
        "--pathologies-path",
        dest="pathologies_path",
        help="Directory path where to save pathologies file",
        type=str,
    )
    argsparser.add_argument(
        "-g",
        "--pathologies-format",
        dest="pathologies_format",
        default="json",
        help="Pathologies file format",
        type=str,
    )
    argsparser.add_argument(
        "-a",
        "--pathologies-filename",
        dest="pathologies_filename",
        default="pathologies.json",
        help="Pathologies file name",
        type=str,
    )
    argsparser.add_argument(
        "-b",
        "--pathologies-encoding",
        dest="pathologies_file_encoding",
        default="utf-8",
        help="Pathologies file encoding",
        type=str,
    )
    argsparser.add_argument(
        "-j",
        "--pathologies-indent-char",
        dest="pathologies_file_indent_char",
        default=" ",
        help="Pathologies file indentation character",
        type=str,
    )
    argsparser.add_argument(
        "-k",
        "--pathologies-indent-count",
        dest="pathologies_file_indent_count",
        default=2,
        help="Pathologies file indentation character count",
        type=int,
    )
    argsparser.add_argument(
        "-n",
        "--pathologies-preserve-dataset-var",
        dest="pathologies_preserve_dataset_var",
        default=False,
        action="store_true",
        help="Preserve dataset variable in pathologies file",
    )
    argsparser.add_argument(
        "-5",
        "--pathologies-mip5-ok",
        dest="pathologies_mip5_compatibility",
        default=False,
        action="store_true",
        help="Generate MIP5 compatible pathologies file",
    )
